...
void battery_Informations()
{
  ...
  switch (batteryType)
  {
    case BATTERY_TYPE_NONE:lcd.print(F("?: "));break;
    case BATTERY_TYPE_NIMH:lcd.print(F("NIMH:"));lcd.print(battery_numberOfCells());lcd.print(F("S"));break;
    case BATTERY_TYPE_LIPO:lcd.print(F("LIPO:"));lcd.print(battery_numberOfCells());lcd.print(F("S"));break;
  }
#endif
#endif
}
...
----'        
  */  
  ...
  uint16_t raw_bat = anaRead(BROCHE_BATTEXT); //attention a I2C qui utilise les pins A4 et A5  
  /* Calcul de la tension reel -- Convert the analog reading (which goes from 0 - 1023) to a voltage (0 - 5V) */
  //float real_bat = ((raw_bat * (5.0 / 1023)) * coeff_division);

  /* If the library VoltageReference.h is used */
  int vcc = vRef.readVcc();
  float real_bat = ((raw_bat * ((vcc/1000*1.0) / 1023)) * coeff_division);
  /* If the library VoltageReference.h is used */
