...
English:
=======
This sketch demonstrates how to use the <RcBusRx> library when it is configured for IBUS.
...
Just connect the Receiver IBUS signal to the Rx pin of the Serial1 UART on the arduino.

Francais:
========
Ce sketch demontre comment utiliser la bibliotheque <RcBusRx> quand elle est configuree en IBUS.
...
Connecter simplement le signal IBUS du recepteur a la broche Rx de l'UART Serial1 de l'arduino.
...
==============
 ...
 |               |              |                      |           |                  | 
 |    IBUS Signal+------------->+ Serial1 Rx           |           | (Serial Console) |
 |               |              |                   USB+-----------+USB               |
 |            Gnd+--------------+ Gnd                  | USB cable |                  |
 |               |              |                      |           |                  |
 '---------------'              '----------------------'           '------------------'
...
void setup()
{
  ...
  Serial.println(F("RcBusRx configured in IBUS receiver\n"));
  
  Serial1.begin(IBUS_RX_SERIAL_CFG); /* Choose your serial and select xxxx__RX_SERIAL_CFG (list in RcBusRx.h) where xxxx is the protocol */
  
  RcBusRx.serialAttach(&Serial1);    /* Then, attach the IBUS receiver to this Serial1 */
  RcBusRx.setProto(RC_BUS_RX_IBUS);  /* Select here the serial protocol */
}
...
void loop()
{
  ...
  if(RcBusRx.isSynchro()) /* One Serial frame just arrived */
  {
    ...
    {
      SerialFrameCnt = 0;
      /* Display IBUS channels and flags in the serial console */
