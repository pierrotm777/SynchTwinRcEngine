...
English:
=======
This sketch demonstrates how to use the <RcBusRx> library when it is configured for JETI EX.
...
Just connect the Receiver JETI EX signal to the Rx pin of the Serial1 UART on the arduino.

Francais:
========
Ce sketch demontre comment utiliser la bibliotheque <RcBusRx> quand elle est configuree en JETI EX.
...
Connecter simplement le signal JETI EX du recepteur a la broche Rx de l'UART Serial1 de l'arduino.
...
==============
 ...
 |               |              |                      |           |                  | 
 |    JETI Signal+------------->+ Serial1 Rx           |           | (Serial Console) |
 |               |              |                   USB+-----------+USB               |
 |            Gnd+--------------+ Gnd                  | USB cable |                  |
 |               |              |                      |           |                  |
 '---------------'              '----------------------'           '------------------'
...
void setup()
{
  ...
  Serial.println(F("RcBusRx configured in JETI EX receiver\n"));
  
  Serial1.begin(JETI_RX_SERIAL_CFG); /* Choose your serial and select xxxx__RX_SERIAL_CFG (list in RcBusRx.h) where xxxx is the protocol */
  
  RcBusRx.serialAttach(&Serial1);    /* Then, attach the JETI receiver to this Serial1 */
  RcBusRx.setProto(RC_BUS_RX_JETI);  /* Select here the serial protocol */
}
...
void loop()
{
  ...
  if(RcBusRx.isSynchro()) /* One Serial frame just arrived */
  {
    ...
    {
      SerialFrameCnt = 0;
      /* Display JETI EX channels and flags in the serial console */
